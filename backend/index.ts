...
app.post('/api/report', async (req, res) => {
  try {
    ...
    if (!vulnerabilities) {
      return res.status(400).json({ error: 'Vulnerabilities are required' });
    }
...
app.post('/api/optimize', async (req, res) => {
  try {
    ...
    if (!fullCode || !vulnerability) {
      return res.status(400).json({ error: 'Full code and vulnerability are required' });
    }
...
app.post('/api/fix', async (req, res) => {
  try {
    ...
    if (!code || !name) {
      return res.status(400).json({ error: 'Code and issue name are required' });
    }
